...
from .utils import LONG_STORY_SHOT_STAT_LABELS_TO_DATA, MAP_FORMAT_TO_EXTRACTOR, AllowedJSONFormatsEnum, parse_int_value
...
class Character(BaseCharacter):
    ...
    def _parse_longstoryshot(cls, data: t.Dict):
        char = Character(
            ...
            sub_info={k: parse_int_value(v["value"]) for k, v in data["subInfo"].items()},
            stats={LONG_STORY_SHOT_STAT_LABELS_TO_DATA[k]: v["score"] for k, v in data["stats"].items()},
            stat_mods={
                LONG_STORY_SHOT_STAT_LABELS_TO_DATA[k]: parse_int_value(v.get("customModifier"))
                for k, v in data["saves"].items()
            },
            skills={k: parse_int_value(v.get("customModifier")) for k, v in data["skills"].items()},
        )
        return char
    ...
    def from_dict(
        cls,
        data: t.Dict,
        format: AllowedJSONFormatsEnum = AllowedJSONFormatsEnum.LONG_STORY_SHOT,
    ) -> BaseCharacter:
    ...
    def from_file(
        ...
        return cls.from_dict(data, format)
